...
class DobotTaskController(Node):
    def __init__(self):
        ...
        # Публішер для масиву маркерів (кубики у сцені)
        self.marker_pub = self.create_publisher(
            MarkerArray, '/visualization_marker_array', 10
        )

        # Публішер для стану суглобів — перекриває joint_state_publisher_gui
        self.joint_pub = self.create_publisher(JointState, '/joint_states', 10)

        self.get_logger().info('Running in Simulation Mode')

        self.timer_rviz = self.create_timer(0.02, self.update_rviz) #створення таймера для виклику функції publish_scene кожну секунду
